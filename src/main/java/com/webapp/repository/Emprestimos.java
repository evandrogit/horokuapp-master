		...
		if (StringUtils.isNotBlank(filter.getNome())) {
			...
			if(filter.getEmprestimoVencido() == true) {			
				typedQuery = manager.createQuery(
						"select e from Emprestimo e join fetch e.cliente c where upper(c.nome) like :nome and e.statusEmprestimo = :status and e.dataVencimento <= :currentDate order by e.id",
			...
			if(filter.getEmprestimoVencido() == true) {	
				typedQuery = manager.createQuery("select e from Emprestimo e where e.statusEmprestimo = :status and e.dataVencimento <= :currentDate", Emprestimo.class).setParameter("status", StatusEmprestimo.ABERTO).setParameter("currentDate", new Date());
